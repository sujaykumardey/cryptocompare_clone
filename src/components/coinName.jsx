...
const CoinMarketDetail = (props) => {
  const tooltipTitleMktCap = `When calculating Market Capitalization (market cap), we account for all coins in circulation, including those held by team members or the company.
    ...
    const getVolume = (message)=> {
        ...
        return   <div className='coin-market-border px-1'>$ {volume.toFixed(2)} M</div>
    }
    return (
    <div className='d-flex '>
      ...
      <div className='pr-2'>
        <div className='badge text-secondary'>Vol. 24H</div>{getVolume(props.response)}
      </div>
      <div className='pr-2'>
...
connect = () => {
    ...
    getPrice = (message)=> {
        ...
        if(oldPrice > message.PRICE){
            return  <div className='pl-2 pb-5 price-dec coin-price-style'>
        <p>$ {message.PRICE}</p>
      </div>
        }
        else if(oldPrice < message.PRICE){
            return  <div className='pl-2  pb-5 price-inc coin-price-style'>
            <p>$ {message.PRICE}</p>
          </div>
        }
        else{
            return  <div className=' pb-5 pl-2 price-same coin-price-style'>
            <p>$ {message.PRICE}</p>
          </div>
        }
    }
    else{
        return  <div className='pl-2 pb-5 coin-price-style'>
        <p>$ {oldPrice}</p>
      </div>
    }
